...
async fn single_node_loopback() {
    ...
    info!("Client created with address: {}", client.public_address_for_tests().to_base58());
    
    // Perform handshake and subscription
    let node_info = client.handshake_with_node(node_addr).await.expect("Handshake failed");
    info!("Handshake successful with node");
    
    client.connected_node = Some(node_info.clone());
    client.subscribe_and_receive_messages(node_addr).await.expect("Subscription failed");
    ...
    info!("Sending message to self at address: {}", pub_addr.to_base58());
    
    client.send_message(pub_addr.clone(), b"hello loop").await.unwrap();
    info!("Message sent");
    ...
    // Verify & decrypt
    let (plaintext, sender_addr) = pkt.verify_and_decrypt(
        client.private_address_for_tests(),
        0,
...
async fn two_node_relay() {
    ...
    let _node_a = Node::new(
        routing_a.clone(), prefix.clone(), addr_a,
        0, 3600, SerializableArgon2Params::default(),
        Duration::from_secs(60), Duration::from_secs(600),
        vec![addr_b], Duration::from_secs(3600)
    ).await;
    
    let _node_b = Node::new(
        routing_b.clone(), prefix.clone(), addr_b,
        0, 3600, SerializableArgon2Params::default(),
        Duration::from_secs(60), Duration::from_secs(600),
        vec![addr_a], Duration::from_secs(3600)
    ).await;
    ...
    let mut client1 = Client::new(
        routing_a.clone(), None, None, 64,
        SerializableArgon2Params::default(), false,
        addr_a
    );
    
    info!("Client1 address: {}", client1.public_address_for_tests().to_base58());
    
    // Handshake and subscribe client1
    let node_info1 = client1.handshake_with_node(addr_a).await.expect("Handshake client1 failed");
    client1.connected_node = Some(node_info1.clone());
    client1.subscribe_and_receive_messages(addr_a).await.expect("Subscription client1 failed");

    // Client2 on nodeB
    let mut client2 = Client::new(
        routing_b.clone(), None, None, 64,
        SerializableArgon2Params::default(), false,
        addr_b
    );
    
    info!("Client2 address: {}", client2.public_address_for_tests().to_base58());
    
    // Handshake and subscribe client2
    let node_info2 = client2.handshake_with_node(addr_b).await.expect("Handshake client2 failed");
    client2.connected_node = Some(node_info2.clone());
    client2.subscribe_and_receive_messages(addr_b).await.expect("Subscription client2 failed");
    ...
    client1.send_message(addr2.clone(), b"relay test").await.unwrap();
    ...
    let (plaintext, sender_addr) = pkt.verify_and_decrypt(
        client2.private_address_for_tests(),
        0,
