...
async fn main() {
    ...
    let bootstrap_node_address: SocketAddr = matches.value_of("bootstrap_node").unwrap().parse().expect("Invalid bootstrap node address");

    let prefix = matches.value_of("prefix").map(|hex_str| {
        let bits = u64::from_str_radix(hex_str, 16).expect("Invalid prefix hex");
        let bit_length = hex_str.len() as u8 * 4; 
        RoutingPrefix { bits: Some(bits), bit_length }
    });
    
    let length = matches.value_of("length").map(|s| s.parse::<u8>().expect("Invalid length"));

    let max_prefix_length: usize = matches.value_of("max_prefix_length").unwrap().parse().expect("Invalid max prefix length");
